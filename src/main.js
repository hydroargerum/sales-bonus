...
function analyzeSalesData(data, options) {
    ...
    if (!data.sellers || data.sellers.length === 0) throw 10;
    ...
    if (!data.products || data.products.length === 0) throw 20;
    ...
    if (!data.purchase_records || data.purchase_records.length === 0) throw 30;
    data.purchase_records.forEach((item, index) => {
        if (!item.seller_id || !item.items || !item.total_amount) {
            ...
            if (!item || item.length === 0) throw 40;
